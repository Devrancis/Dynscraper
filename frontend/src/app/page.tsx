...
export default function Dashboard() {
  ...
  const filteredIntel = useMemo(() => {
    return intel
      .filter((item) => item.indicator.toLowerCase().includes(searchTerm.toLowerCase()))
      .filter((item) => filterSource === "all" || item.scan_target.includes(filterSource));
  }, [intel, searchTerm, filterSource]);

  // ─── FIX 1: Derive stats directly from the raw ISO string, not a pre-formatted string ───
  const totalThreats = intel.length;
  const latestScrapeISO = intel.length > 0 ? intel[0].scraped_at : null;
  ...
  // ─── FIX 2: Skeleton values — only show real data once loading is done ───
  const statCards = [
    {
      title: "Database Integrity",
      // Show skeleton text while loading, real value after
      value: loading ? null : `${totalThreats} Indicators`,
      subtitle: "Total Unique Threats",
      icon: "󱁊",
    },
    {
      title: "Primary Target",
      value: loading ? null : "Hacker News",
      subtitle: "Active Scan Target",
      icon: "󰆂",
    },
    {
      title: "Last Operations Cycle",
      // FIX 1 applied here: use the raw ISO string directly, guard null
      value: loading ? null : latestScrapeISO ? formatTacticalTime(latestScrapeISO) : "No data yet",
      subtitle: "Previous Scrape Time",
      icon: "󱎬",
    },
    {
      title: "System Uptime",
      // FIX 3: null means still checking — don't show OFFLINE until we actually know
      value: loading || apiOnline === null ? null : apiOnline ? "STABLE" : "OFFLINE",
      subtitle: "Core API Status",
      icon: apiOnline === true ? "󰒋" : apiOnline === false ? "󰅙" : "󰒋",
      color: apiOnline === true ? "text-emerald-500" : apiOnline === false ? "text-red-500" : "text-slate-500",
    },
  ];
        ...
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="text-xl font-black text-emerald-500 tracking-tighter">
              XPECTRA<span className="text-emerald-300">SCRAPE</span>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 space-y-10">
        ...
        <div className="p-8 bg-slate-900 rounded-xl border border-slate-800 shadow-[0_0_60px_-15px_rgba(16,185,129,0.15)] flex flex-col gap-6">
          <div className="flex flex-col md:flex-row gap-10 items-end justify-between">
            ...
            >
              ...
              <span className={isScraping ? "animate-spin" : ""}>{isScraping ? "󰀕" : "󱕔"}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {statCards.map((stat, idx) => (
            <div
              key={idx}
              className="bg-slate-900 p-6 rounded-lg border border-slate-800 space-y-2 hover:border-emerald-800/50 hover:bg-slate-800/40 transition-all cursor-default shadow-lg"
            >
              ...
              {/* ─── Skeleton pulse while value is null (loading) ─── */}
              {stat.value === null ? (
